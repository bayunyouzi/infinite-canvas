...
type ChatStreamState = { buffer: string; text: string; toolCallMap: Map<number, { id: string; name: string; args: string }>; error?: string };
...
// 将 ResponseInputMessage 转换为 Chat Completions messages 格式
        ...
        if ("type" in message) {
            if (message.type === "function_call") {
                return [{
                    role: "assistant",
                    content: null,
                    tool_calls: [{ id: message.call_id, type: "function", function: { name: message.name, arguments: message.arguments } }],
                }];
            }
            return [];
        }
...
function toChatToolChoice(toolChoice: "auto" | "required" | { type: "function"; name: string }) {
...
function consumeChatStreamBlock(block: string, state: ChatStreamState, onDelta?: (text: string) => void) {
    const data = block
        ...
        .trim();
    if (!data || data === "[DONE]") return;
    let event: ChatCompletionsPayload;
    try {
        event = JSON.parse(data) as ChatCompletionsPayload;
    } catch {
        return;
    }
    const errorMsg = responseErrorMessage(event);
    if (errorMsg) { state.error = errorMsg; return; }
    ...
    if (Array.isArray(delta.tool_calls)) {
        for (const tc of delta.tool_calls) {
            ...
            if (!state.toolCallMap.has(idx)) {
                state.toolCallMap.set(idx, { id: tc.id || "", name: tc.function?.name || "", args: "" });
            }
            const entry = state.toolCallMap.get(idx)!;
...
async function requestChatCompletions(
    ...
    if (tools.length > 0) {
        body.tools = tools;
        body.tool_choice = toChatToolChoice(toolChoice);
    ...
    const state: ChatStreamState = { buffer: "", text: "", toolCallMap: new Map() };
    for (;;) {
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    try {
        ...
        const chatMessages = tochatMessages(withSystemMessage(requestConfig, messages));
        const answer = (await requestChatCompletions(requestConfig, chatMessages, [], "auto", onDelta, options)).content || "没有返回内容";
        if (answer === "没有返回内容") onDelta(answer);
        return answer;
    } catch (error) {
        throw new Error(readAxiosError(error, "请求失败"));
...
export async function requestToolResponse(config: AiConfig, messages: ResponseInputMessage[], tools: ResponseFunctionTool[], toolChoice: ToolChoice = "auto", onDelta?: (text: string) => void, options?: RequestOptions): Promise<ToolResponseResult> {
    ...
    try {
        ...
        return await requestChatCompletions(requestConfig, chatMessages, chatTools, toolChoice, onDelta, options);
    } catch (error) {
        throw new Error(readAxiosError(error, "请求失败"));
    }
}

export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat" | "proxyMode">) {
    try {
        ...
        const response = await axios.get<{ data?: Array<{ id?: string }>; error?: { message?: string } }>(buildApiUrl(config.baseUrl, "/models", config.proxyMode), {
            headers: aiHeaders(config as AiConfig),
        });
...
export async function fetchChannelModels(channel: ModelChannel, proxyMode?: AiConfig["proxyMode"]) {
    return fetchImageModels({ baseUrl: channel.baseUrl, apiKey: channel.apiKey, apiFormat: channel.apiFormat, proxyMode: proxyMode || "direct" });
}
